...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    // Não retornar `true` desnecessariamente para evitar erros de canal de mensagem.
});
...
async function processCompletedSession(sessionData) {
    ...
    // 2. Lógica de crescimento das plantas (CORRIGIDA)
    const originalLayout = data.gardenLayout || [];
    const updatedLayout = [];

    for (const item of originalLayout) {
        // Cria uma cópia profunda para evitar mutação
        const newItem = JSON.parse(JSON.stringify(item));

        if (newItem.type === 'plant' && !newItem.withered && newItem.growthStage < GROWTH_STAGES - 1) {
            newItem.growthStage = (newItem.growthStage || 0) + 1;
        }
