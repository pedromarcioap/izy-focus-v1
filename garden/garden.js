document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadData() {
        if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
            ...
            console.log("Chrome storage not available. Using default empty data.");
            // Initialize with default empty data for testing or non-extension environments
            gardenData.layout = [];
            gardenData.inventory = { seeds: { 'Padrão': 5 }, stones: 3 };
        }
    }
    ...
    function renderGarden() {
        Array.from(gardenGrid.children).forEach(cell => {
            cell.innerHTML = ''; // Clear only the cell content
        });
    ...
    function createGardenItemElement(itemData) {
        ...
        itemElement.draggable = isEditMode; // Only draggable in edit mode
        itemElement.dataset.itemType = itemData.type;
        itemElement.dataset.position = itemData.position;

        if (itemData.type === 'plant') {
            itemElement.dataset.seedType = itemData.seedType;
            if (itemData.withered) {
                ...
                itemElement.addEventListener('click', () => handleWitheredPlantClick(itemData));
            } else {
                const stage = itemData.growthStage || 0;
                const normalizedSeedType = (itemData.seedType || 'Padrao').normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                itemElement.style.backgroundImage = `url('../assets/icons/${normalizedSeedType}_plant_stage${stage}.png')`;
            }
        ...
        if (gardenData.inventory && typeof gardenData.inventory.seeds === 'object') {
            for (const [seedType, count] of Object.entries(gardenData.inventory.seeds)) {
                if (count > 0) {
                    ...
                    li.draggable = true; // Inventory items are always draggable
